...
object ExploringForEach {
  def main(args: Array[String]): Unit = {
    ...
    "Hello Worlds".foreach(x => print(x.toUpper))
    ...
    (100 to 150 by 10).foreach(x => print(x + " "))
    
    println()
    
    List.range(100, 110).foreach(x => {
      print((x * 3) + " ")
    })
    
    println()
    
    var anim = Array(Array("dog", "cat", "fish"), Array("wolf", "lion", "shark"))
    anim.foreach(arr => arr.foreach(elem => print(elem)))
  }
}