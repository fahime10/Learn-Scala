...
object ExploringFlatMap {
  def main(args: Array[String]): Unit = {
    ...
    (1 to 5).flatMap(i => (1 to i).map(j => j * 2)).foreach(x => print(x + " ")) // it's giving a range(1, 22, 333, 4444, 55555)
    ...
    (1 to 5).flatMap(i => (1 to i).map(j => i * j)).foreach(x => print(x + " "))  // this is another range(1*1, 2*1 and 2*2, 3*1 and 3*2 and 3*3, etc,)
  }
}